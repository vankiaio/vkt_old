...
BOOST_FIXTURE_TEST_CASE(multiple_producer_votepay_share, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   std::vector<account_name> producer_names;
   {
      ...
      for (const auto& p: producer_names) {
         ...
         BOOST_TEST_REQUIRE(0 == get_producer_info(p)["votepay_share"].as_double());
         BOOST_TEST_REQUIRE(0 == get_producer_info(p)["last_claim_time"].as_uint64());
         BOOST_TEST_REQUIRE(0 == get_producer_info(p)["last_votepay_share_update"].as_uint64());
      }
   }
   ...
   // producvoterd votes for abcproducera ... abcproducern
   {
      ...
      uint64_t init_update = init_info["last_votepay_share_update"].as_uint64();
      double   init_votes  = init_info["total_votes"].as_double();
      BOOST_REQUIRE_EQUAL( success(), vote(N(producvoterb), vector<account_name>(producer_names.begin(), producer_names.begin()+21)) );
      const auto& info = get_producer_info(producer_names[0]);
      BOOST_TEST_REQUIRE( ((info["last_votepay_share_update"].as_uint64() - init_update)/double(1E6)) * init_votes == info["votepay_share"].as_double() );
      BOOST_TEST_REQUIRE( info["votepay_share"].as_double() * 10, get_global_state()["total_producer_votepay_share"].as_double() );
      BOOST_TEST_REQUIRE( 0 == get_producer_info(producer_names[11])["votepay_share"].as_double() );
      produce_block( fc::hours(43) );
      BOOST_REQUIRE_EQUAL( success(), vote(N(producvoterc), vector<account_name>(producer_names.begin(), producer_names.begin()+26)) );
      BOOST_REQUIRE( 0 < get_producer_info(producer_names[11])["votepay_share"].as_double() );
      BOOST_REQUIRE_EQUAL( success(), vote(N(producvoterd), vector<account_name>(producer_names.begin()+26, producer_names.end())) );
      BOOST_TEST_REQUIRE( 0 == get_producer_info(producer_names[26])["votepay_share"].as_double() );
   }
   ...
   std::vector<double> votepay_shares(producer_names.size());
   {
      ...
      for (uint32_t i = 0; i < producer_names.size(); ++i) {
         votepay_shares[i] = get_producer_info(producer_names[i])["votepay_share"].as_double();
         total_votepay_shares += votepay_shares[i];
      }
      BOOST_TEST_REQUIRE( get_global_state()["total_producer_votepay_share"].as_double() == total_votepay_shares );
   }
   
   {
      ...
      const auto& init_global_state = get_global_state();
      BOOST_REQUIRE_EQUAL( 0, init_info["last_claim_time"].as_uint64() );
      BOOST_REQUIRE( 0 < init_info["votepay_share"].as_double() );
      BOOST_REQUIRE( 0 < init_info["last_votepay_share_update"].as_uint64() );
